...
One bounded exception exists for workspace attestation. An agent's direct manager may call the legacy heartbeat invoke route for that report only when the request supplies an issue id, an explicit `resumeFromRunId`, a fresh idempotency key, and no fresh-session override. The heartbeat service revalidates that the prior run belongs to the target agent, the blocked issue is assigned to that agent, the caller is still its direct manager, and unresolved blockers still exist. For a project-scoped target, the source run's issue must also remain bound to a same-project execution workspace whose persisted source is `task_session`, mode is isolated, and strategy is `git_worktree`; the attestation run derives a fresh isolated workspace from that verified source and records the derivation. A permitted run is marked as a dependency-blocked task-session attestation: it may materialize and inspect the resumed issue-scoped workspace, but it does not checkout the issue, change its status or assignee, or satisfy/remove any blocker. Any target, actor, assignment, method, session, source-workspace, or dependency-state mismatch is recorded as a skipped wake and creates no run.
